...
public:
    int myAtoi(string s) {
        ...
        int *intArr = new int[length + 1];          // 第一位表示正负号 (1是负数，0是正数)
        intArr[0] = 0;                              // 预设正数
        int intLength = 0;                          // 正负号同样占一位
        ...
        for (int i = 0; i < length; ++i) {
            if (intLength == 0) {
                ...
                intArr[intLength++] = data[i] - 48;
            } else {
                break;
            }
        }

        long long absoluteValue = 0;
        ...
        for (int i = 1; i < intLength; ++i) {
            ...
            if(intLength - i - 1 > 11){
                absoluteValue = (one << 31);
                break;
            }
            absoluteValue += ((long long) pow(10, intLength - i - 1)) * intArr[i];
        }
        // 溢出判断
        if (intArr[0] == 0 && absoluteValue > (one << 31) - 1) {
            return (int) ((one << 31) - 1);
        } else if (intArr[0] != 0 && absoluteValue > (one << 31)) {
            return (int) ((one << 31) * -1);
        } else {
            return (int) (intArr[0] == 0 ? absoluteValue : -1 * absoluteValue);
        }
    }
};
...
int main() {
    ...
    assert(solution.myAtoi("0004193 with words") == 4193);
